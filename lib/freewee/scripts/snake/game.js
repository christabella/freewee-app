...
var SnakeGame = {
    ...
    update:function() {
        ...
        for (var i = 0; i < numPlayers; i++) {
            if (gotUpdate || globalMic[i]) {
                this.actionOnClick(globalButtons[i], globalMic[i]);
            }            
        }
        gotUpdate = false;
    ...
    actionOnClick:function (buttons, mic){ //once user clicks on button 
      for (var i=0;i<numPlayers;i++){
            //play sumo animation 
        ...
        if (buttons.indexOf(master.toString()) >= 0 && mic) {
