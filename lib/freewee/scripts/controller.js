...
sync.onJoin(function(data, err){
  if (!err){
    ...
    if ($('.displaysumobg').css('background-image') == null) {
  ...
  if (!err){
    ...
    if (data.gameId === 3) {
      ...
      $('#moobs').on( 'touchstart', function( event ) {
        setTimeout(function() {$('#head-crotch').css('visibility', 'visible');}, 15000);
        setTimeout(function() {$('#head-crotch').css('visibility', 'visible');}, 30000);
        $( this ).off( event );
      });
    }
  } else {
